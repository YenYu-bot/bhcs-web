...
- 原生成器的 unit 呼叫以 parser 抽至配對括號結尾；除 radians/quadrant 移除錯誤軸上角 3π/2 外，六個 unit 原文逐位元組不變，3,600／3,600 次固定種子重播相同。象限 unit 的 600 次差異另列於下。新舊混合整卷的抽題序列會改變。
...
原 radians/quadrant 角度池共 13 個，唯一軸上角是 3π/2；0、π/2、π、2π 都不在該池。3π/2 的終邊在負 y 軸，舊答案卻寫第四象限，因此移除這一項，保留其他 12 個角。這會刪除 `qu:3/2:k`（k＝−4…4）九種舊 sig。固定種子每級 200 題共 600 次原分支呼叫，舊版實際抽到錯題 30 次；新版相對舊版改變 180 題，其中 150 題是池長由 13 變 12 後同一亂數索引映到其他合法角；其餘 420 題相同。舊版命中錯題的 sig：`qu:3/2:-1`, `qu:3/2:-2`, `qu:3/2:-3`, `qu:3/2:-4`, `qu:3/2:0`, `qu:3/2:1`, `qu:3/2:4`。每題前後 sig 與位置詳見 results.json 的 axisReplay.changedSigs。例如基礎級第 119 次舊題 `qu:3/2:0`（3π/2，被錯標第四象限）在新版成為 `qu:4/3:0`：判斷角 4π/3 的終邊位於第幾象限，並寫出 [0，2π) 內同界角；答案為第三象限、4π/3。

另外將 realAngle 題幹改為「θ＝67/6（弧度）」格式，抽球的至少紅球數限制為 0＜下限＜抽球數。掃描 PR-C 新題題幹的括號說明，側面積題改用「求以 π 表示的側面積」；其餘括號是數學標示，不再說明生成方式。
...
題幹：袋中有紅球 4 顆、藍球 2 顆，每球等可能被抽到。同時抽 3 球且不放回，求至少 2 顆紅球的機率。

教用答案：**4/5**。至少 2 顆紅球，可能有 2 或 3 顆：[C(4,2)C(2,1)＋C(4,3)]／C(6,3)＝16/20＝4/5。

### probability/balls/threeColors｜三色抽球至少條件（挑戰）

題幹：袋中有紅球 8 顆、藍球 6 顆、白球 4 顆，每球等可能被抽到。同時抽 3 球且不放回，求至少 1 顆紅球的機率。

教用答案：**29/34**。依紅球數分成 2、3、4 顆：[C(4,2)C(3,2)＋C(4,3)C(3,1)＋C(4,4)]／C(7,4)＝31/35。

### radians/quadrant/realAngle｜實數弧度的象限

題幹：角 θ＝67/6（弧度），判斷其終邊位於第幾象限。
...
題幹：直圓錐底面半徑 4 又 2/3、母線長 7，求以 π 表示的側面積。
