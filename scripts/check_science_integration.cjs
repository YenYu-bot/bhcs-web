...
// 2026-09-21：使用者核准數學總覽頁品牌化（僅外觀、搜尋與年級跳轉；91 個工具連結，下方另有斷言）。
...
// P1 handoff explicitly authorizes screen branding of all 91 linked worksheets.
...
{const mathIndex=fs.readFileSync(path.join(root,'tools/math/index.html'),'utf8');assert.equal((mathIndex.match(/class="card"/g)||[]).length,91,'math index must keep all 91 tool links');}
