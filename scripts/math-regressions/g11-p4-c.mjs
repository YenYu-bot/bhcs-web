...
for(const [topic,ids]of Object.entries(targets))for(const id of ids){
 ...
 const node=parseExpressionAt(src,m.index,{ecmaVersion:'latest'});const original=src.slice(node.start,node.end);
 if(topic==='radians'&&id==='quadrant'){
  const axis='R(4,3),R(3,2),R(5,3)';assert.equal(original.split(axis).length,2);
  assert.ok(after.get(topic).includes(original.replace(axis,'R(4,3),R(5,3)')),'only 3π/2 may be removed from original quadrant generator');
 }else assert.ok(after.get(topic).includes(original),`${topic}/${id}: legacy source changed`);
}
...
let replays=0,papers=0,questions=0,legacyPromptRepeats=0;const axisReplay={checked:0,changed:0,oldAxis:0,changedSigs:[],oldAxisSigs:new Set()};const samples={},rows=[],gateRows=[];
...
for(const [topic,ids]of Object.entries(targets))for(const id of ids){
 for(const level of ['basic','advanced','challenge']){
  ...
  for(let i=0;i<200;i++){const a=o.gen(ctx),b=u.gen(ctx);
   if(topic==='radians'&&id==='quadrant'){
    axisReplay.checked++;if(a?.sig?.startsWith('qu:3/2:')){axisReplay.oldAxis++;axisReplay.oldAxisSigs.add(a.sig);}
    if(JSON.stringify(a)!==JSON.stringify(b)){axisReplay.changed++;axisReplay.changedSigs.push({level,index:i,before:a?.sig,after:b?.sig});}
    assert.ok(!b?.sig?.startsWith('qu:3/2:'),'axis angle remains in quadrant pool');
    if(b)assert.equal(b.verify(),true);
   }else{assert.equal(JSON.stringify(a),JSON.stringify(b),`${topic}/${id}/${level} replay ${i}`);if(a)assert.equal(a.verify(),b.verify());replays++;}
  }
 }
 ...
 for(const level of ['basic','advanced','challenge'])for(const mode of load('g11',topic).CFG.units.find(u=>u.id===id).modes){
  for(const size of [40,80])for(let seed=1;seed<=seeds;seed++){
   ...
   for(let i=0;i<size;i++){
    ...
    if(isNew){
     assert.ok(!/不含 π 因子|保留 π/.test(q.expr),'generator explanation in prompt');
     if(topic==='probability'&&id==='balls'&&/p4c:balls:(?:atLeast|threeColors):/.test(q.sig)){
      const m=text.match(/同時抽 (\d+) 球.*求至少 (\d+) 顆紅球/);assert.ok(m,`missing atLeast counts: ${text}`);
      assert.ok(Number(m[2])>0&&Number(m[2])<Number(m[1]),`degenerate atLeast: ${text}`);
     }
...
const result={passed:true,axisReplay:{...axisReplay,oldAxisSigs:[...axisReplay.oldAxisSigs].sort()},unchangedUnits,replays,papers,questions,legacyPromptRepeats,rows,gateRows,samples};
if(process.env.P4_C_REPORT)fs.writeFileSync(process.env.P4_C_REPORT,JSON.stringify(result,null,2)+'\n');
console.log(JSON.stringify({test:'g11-p4-c',passed:true,axisReplay:{checked:axisReplay.checked,changed:axisReplay.changed,oldAxis:axisReplay.oldAxis},unchangedUnits,replays,papers,questions,legacyPromptRepeats,newKinds:Object.keys(samples).length}));
