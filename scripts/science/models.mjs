...
export function describe(id,s,r){
 ...
 case 'electromagnetism':return {metrics:s.mode==='magnet'?[['線圈內磁場',f(r.field*1000,4)+' mT'],['固定線圈長度','0.20 m'],['電流',f(s.current)+' A'],['方向',r.kind==='zero'?'無場':r.kind==='positive'?'正向':'反向']]:s.mode==='motor'?[['當下轉矩',f(r.torque,4)+' N·m'],['線圈面積','0.01 m²'],['法線夾角',s.angle+'°'],['轉速','未由模型求解']]:[['當下感應電壓',f(r.emf,4)+' V'],['匝數×磁通量',f(r.flux,4)+' Wb·匝'],['角速度',f(r.omega)+' rad/s'],['旋轉頻率',f(s.rpm/60)+' Hz']],explanation:s.mode==='magnet'?'固定線圈長度下，磁場與匝數、電流成正比；負電流把方向反轉。':s.mode==='motor'?'當下转矩與sinθ相關。本圖不模擬直流換向器；完整換向說明可看相關馬達教材。':'法線朝向磁場時磁通量最大，卻在這一瞬間變化率為零；轉過90°時磁通量為零，但變化率最大。'};
 ...
 case 'solubility':return {metrics:[['最多可溶解',f(r.capacity)+' g'],['已溶解',f(r.dissolved)+' g'],['未溶固體',f(r.solid)+' g'],['質量百分濃度',f(r.percent)+' %']],explanation:`模型${s.solute.toUpperCase()}在${s.temperature}°C的溶解度是${f(r.solubility)}g／100g水。濃度只計入已溶解的${f(r.dissolved)}g，不把${f(r.solid)}g固體加进分母。`};
 ...
 case 'moon-eclipse':return {metrics:[['可見亮面',f(r.lit*100)+' %'],['月球黃緯',f(r.latitude)+'°'],['相位角',f(r.phase)+'°'],['對齊判斷',r.kind==='solar'?'日食候選':r.kind==='lunar'?'月食候選':'不符合食門檻']],explanation:'月相由觀測方向相對受光半球決定；日月食还需要接近軌道交點。候選狀態只表示符合本頁簡化門檻，不能當作日食或月食預報。'};
