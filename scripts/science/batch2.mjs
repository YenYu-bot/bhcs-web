...
export const batch2 = [
 ...
 controls:[select('mode','裝置',[['magnet','空心電磁鐵'],['motor','線圈轉矩'],['generator','交流發電機']],'magnet'),range('turns','線圈匝數',10,200,10,100,'匝'),range('current','電流（電磁鐵／轉矩）',-2,2,0.1,1,'A'),range('field','外加磁場（轉矩／發電）',-0.5,0.5,0.05,0.2,'T'),range('angle','法線與磁場夾角（轉矩／發電）',0,360,5,90,'°'),range('rpm','轉速（發電）',0,600,10,60,'rpm')],
 prediction:'主要輸出（磁場／轉矩／感應電壓）的正負為何？',choices:[['positive','正方向'],['zero','零'],['negative','負方向']],
 tasks:[task('① 電流反向',{mode:'magnet',turns:100,current:1},'比較+1A與−1A；匝數不變時磁場方向與大小怎麼變？'),task('② 線圈轉矩',{mode:'motor',angle:0,current:1,field:0.2,turns:100},'比較法線與磁場夾角0°、90°、180°。'),task('③ 停止轉動',{mode:'generator',angle:90,field:0.2,rpm:0,turns:100},'從靜止到60rpm；再把磁場反向，比較感應電壓。')],
 formula:'空心長螺線管 B=μ₀NI/ℓ（ℓ=0.20m）；τ=NBIA sinθ；ε=NBAω sinθ，ω=2π rpm/60，A=0.01m²。',
 limits:'電磁鐵採長螺線管近似；不計漏磁、鐵芯、電阻、發熱與反電動勢。馬達頁顯示當下轉矩而非自行推算轉速；發電機的转速由外力維持。正負只是固定繞線及法線約定，不是所有實際馬達的絕對方向。各模式只使用標示適用的控制。',
 ...
 quiz:[quiz('空心線圈電流反向，磁場？',['消失','方向反向','大小必定加倍'],1,'比较+I和−I。'),quiz('線圈法線平行磁場時轉矩？',['最大','無限大','零'],2,'此時 θ=0，sinθ=0。'),quiz('磁場固定，線圈完全停止且無其他變化時？',['沒有感應電壓','仍持續最大電壓','電壓隨時間增加'],0,'發電需要磁通量隨時間改變。')]
 },
 {
 ...
 limits:'圓軌道教學幾何；相位角近似黃經差，並非精密星曆。以離朔／望≤5°且|β|≤1°標記「食候選」，門檻只作教學，不能預測日期、食分、全食環食或地面可見地區。軌道俯視圖刻意放大天體，垂直偏离以讀值呈現。',
 ...
 goals:['用地軸傾斜解釋季節','比较不同緯度的正午太陽高度','辨識極晝極夜與邊界'],
 ...
 tasks:[task('① 臺中夏冬比較',{latitude:24,season:90,tilt:23.5},'记录北半球夏至，再把位置改到270°，比較正午高度與日長。'),task('② 南北半球',{latitude:-24,season:90,tilt:23.5},'同一公轉位置，緯度正負交換；季節相同嗎？'),task('③ 極圈與地軸',{latitude:70,season:90,tilt:23.5},'觀察70°N的日長，再把傾角調到0°。')],
 ...
 quiz:[quiz('黑暗中植物通常仍會？',['只做光合作用','進行呼吸作用','不進行任何代謝'],1,'光照0%时总光合为0，呼吸项仍在。'),quiz('其他條件相同、氣孔開度固定，空氣濕度越高，蒸散指標？',['較低','較高','一定不變'],0,'比較水氣差異，而不是把蒸散和呼吸當成一件事。'),quiz('本頁的氣孔開度與蒸散值屬於？',['植物實測資料','所有植物通用常數','簡化教學設定與相對指標'],2,'真實植物還有水分壓力與氣孔回饋。')]
 },
 {
