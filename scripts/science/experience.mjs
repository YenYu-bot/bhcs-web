...
const firstGuides={
 'microscope-lab':['高倍時，為什麼看見的細胞反而變少？','先選字母 e 玻片，用 4× 物鏡調清楚，再對照試片上的正寫 e。','拖動玻璃試片，看倒轉的 e 往哪裡移。試著把 e 移回十字中央，再換 10×、40× 比較。','各記錄一筆低倍與高倍結果，說出視野直徑和比例尺的差異。','experiment'],
 'genetics-simulation-lab':['兩株高莖豌豆，能生出矮莖子代嗎？','保留「莖高」與兩個異合親代。Tt 表示一個 T、一個 t；每個配子只帶其中一個。','完成雜交。點四格中的任一格，看兩個配子如何形成子代。','抽樣 4 個，再抽樣 1000 個：比較實際比例與理論比例，不只背 3：1。','crossLab'],
 'force-motion-lab':['相同的推力，為什麼加速程度可能不同？','先保留預設值，按開始觀察加速度方向。注意「速度方向」不一定等於「加速度方向」。','上方看車的位置，下方看分開排列的施力、摩擦與合力；可暫停，再逐步前進。','第一筆記錄後，只改質量或接觸面，再比較合力與加速度。','experiment'],
 'circuit-lab':['增加一顆燈泡，總電流一定變小嗎？','先用單一燈泡，保留電壓與電阻，按「通電驗證」。','只改接法為串聯，再改並聯；每次讀取總電流和各燈電壓。','保留三筆紀錄，指出串聯和並聯的電流路徑差在哪裡。','experiment'],
 'particle-reaction-lab':['化學反應後，原子有沒有變少？','選擇反應，先看兩種反應物的分子數，再開始反應並確認哪一種先用完。','開始反應，對照反應前後每種顏色代表的原子數與剩餘分子。','只增加一種反應物再比較，說明為何反應物增加卻不一定產生更多產物。','experiment'],
 'plate-earthquake-lab':['地震從哪裡開始，又如何找出位置？','先選一種板塊邊界，再觀察兩側移動方向與形成的地形。','比較張裂、聚合、錯動；再到震央定位，用測站資料判斷距離。','找出多個距離圓的交會區，分清震源與地表上的震央。','experiment'],
 'heat-phase-lab':['一直加熱，溫度就會一直上升嗎？','先看目前物態，再開始觀察加熱曲線與物態變化。','對照曲線斜段與水平段，同時讀取溫度、物態和吸收能量。','各記錄一次升溫與相變，說明能量增加時溫度是否一定改變。','experiment'],
 'buoyancy-density-lab':['物體很重，就一定會沉下去嗎？','選定物體與液體，放入液體觀察浮沉。','比較重力、浮力與排開液體；只改密度或體積，其他條件固定。','記錄兩組結果，用密度和受力解釋，不只用大小猜浮沉。','experiment'],
 'acid-base-indicator-lab':['兩杯液體混合，怎麼判斷最後酸鹼？','讀取酸與鹼的濃度和體積，選指示劑，再混合觀察結果。','混合後對照 pH、顏色與剩餘酸鹼；不要只比較兩杯體積。','只改其中一個條件並重新記錄，用莫耳數差異解釋。','experiment'],
 'photosynthesis-factor-lab':['燈越亮，植物的光合作用就一定越快嗎？','保留二氧化碳與溫度，只改光照，直接觀察速率變化。','開始觀察，記下速率，再只增加光照；注意是否已接近平台。','改變二氧化碳再比較，說明哪個因素限制了目前的速率。','experiment'],
};
function route(question,steps,links){return `<aside class="lesson-route" aria-labelledby="self-study-title"><h2 id="self-study-title">自學起點：${escape(question)}</h2><p>第一次使用，照這三步做；直接從操作與觀察留下可以比較的證據。</p><ol class="route-steps">${steps.map((s,i)=>`<li><b>${['01　設定研究條件','02　操作與觀察','03　比較與解釋'][i]}</b><p>${escape(s)}</p><a href="${links[i]}">${['前往操作','看觀察區','前往檢核'][i]} →</a></li>`).join('')}</ol></aside>`}
...
export function enhanceLessons(root,firstBatch,batch2){
 ...
 for(const item of [...firstBatch.map(file=>({file,id:file.replace('.html','')})),...batch2.map(t=>({...t,file:'science/'+t.id+'.html'}))]){
  ...
  let guide=known?route(known[0],known.slice(1,4),['#'+known[4],'#'+known[4],'#quiz']):route(item.goals[0]+'，從哪裡開始？',[
   ...
   '設定好條件，按「操作並觀察」。對照圖解、讀值與下方說明；不知道術語時，先讀本頁的常見迷思。',
  ...
  const study=`<details class="inquiry-guide" id="inquiry-guide"><summary>本課比較任務：固定什麼、只改什麼？</summary><dl>${lesson.slice(0,4).map((s,i)=>`<div><dt>${['保持不變','只改一個條件','記錄證據','完成標準'][i]}</dt><dd>${escape(s)}</dd></div>`).join('')}</dl><label for="study-note">用自己的話解釋（可選填，勿填姓名）</label><textarea id="study-note" rows="3" maxlength="1000" placeholder="我固定了＿＿，只改＿＿；觀察到＿＿，我的解釋是＿＿。"></textarea><details class="teacher-plan"><summary>教師帶領：15 分鐘比較活動</summary><ol><li>3 分鐘：讀「保持不變」與「只改一個條件」，確認這次比較的條件。</li><li>7 分鐘：完成兩筆觀察。${escape(lesson[4])}</li><li>5 分鐘：同學交換證據，以「完成標準」檢查解釋，再做頁面檢核。</li></ol><p>若學生仍卡住：先請他指著畫面讀出一個數值，再說另一組如何不同，不必一次背出全部術語。</p></details></details><fieldset class="study-checks"><legend>我的自學進度（自己確認，不代表測驗成績）</legend>${['已完成第一筆操作並說出看到什麼','已完成兩組比較','能引用證據解釋差異'].map((s,i)=>`<label><input type="checkbox" data-study-step="${i}">${s}</label>`).join('')}<p id="study-progress" role="status" aria-live="polite">已確認 0／3 項</p></fieldset><div class="study-storage"><span id="study-save-status">進度與筆記只儲存在這台瀏覽器。</span><button type="button" id="study-clear">清除自學進度與筆記</button></div>`;
