...
const firstGuides={
 ...
 'genetics-simulation-lab':['兩株高莖豌豆，能生出矮莖子代嗎？','保留「莖高」與兩個異合親代。Tt 表示一個 T、一個 t；每個配子只帶其中一個。','完成雜交後，點四格中的任一格，看兩個配子如何形成子代。','抽樣 4 個，再抽樣 1000 個：比較實際比例與理論比例，不只背 3：1。','crossLab'],
 'force-motion-lab':['相同的推力，為什麼加速程度可能不同？','先保留預設值並按開始。注意「速度方向」不一定等於「加速度方向」。','上方看車的位置，下方看分開排列的施力、摩擦與合力；可暫停，再逐步前進。','第一筆記錄後，只改質量或接觸面，再比較合力與加速度。','experiment'],
 'circuit-lab':['增加一顆燈泡，總電流一定變小嗎？','先用單一燈泡，保留電壓與電阻，按「通電驗證」。','只改接法為串聯，再改並聯；每次讀取總電流和各燈電壓。','保留三筆紀錄，指出串聯和並聯的電流路徑差在哪裡。','experiment'],
 'particle-reaction-lab':['化學反應後，原子有沒有變少？','選擇反應，先看兩種反應物的分子數，再開始反應並觀察哪一種先用完。','開始反應，對照反應前後每種顏色代表的原子數與剩餘分子。','只增加一種反應物再比較，說明為何反應物增加卻不一定產生更多產物。','experiment'],
 'plate-earthquake-lab':['地震從哪裡開始，又如何找出位置？','先選一種板塊邊界，直接觀察兩側移動方向與形成地形。','比較張裂、聚合、錯動；再到震央定位，用測站資料判斷距離。','找出多個距離圓的交會區，分清震源與地表上的震央。','experiment'],
 'heat-phase-lab':['一直加熱，溫度就會一直上升嗎？','先看目前物態，開始模擬並觀察加熱曲線。','對照曲線斜段與水平段，同時讀取溫度、物態和吸收能量。','各記錄一次升溫與相變，說明能量增加時溫度是否一定改變。','experiment'],
 'buoyancy-density-lab':['物體很重，就一定會沉下去嗎？','選定物體與液體，放入液體後直接觀察浮沉與浸入比例。','比較重力、浮力與排開液體；只改密度或體積，其他條件固定。','記錄兩組結果，用密度和受力解釋，不只用大小猜浮沉。','experiment'],
 'acid-base-indicator-lab':['兩杯液體混合，怎麼判斷最後酸鹼？','讀取酸與鹼的濃度和體積，選指示劑，直接開始混合並觀察結果。','混合後對照 pH、顏色與剩餘酸鹼；不要只比較兩杯體積。','只改其中一個條件，重新操作並記錄，用莫耳數差異解釋。','experiment'],
...
export function enhanceLessons(root,firstBatch,batch2){
 ...
 for(const item of [...firstBatch.map(file=>({file,id:file.replace('.html','')})),...batch2.map(t=>({...t,file:'science/'+t.id+'.html'}))]){
  ...
  const predictionFree=["microscope-lab","genetics-simulation-lab","force-motion-lab","circuit-lab","particle-reaction-lab","plate-earthquake-lab","heat-phase-lab","buoyancy-density-lab","acid-base-indicator-lab","photosynthesis-factor-lab"].includes(item.id)||item.noPrediction===true;
