(() => {
 ...
 const predictionSet=()=>mission===null?{question:conf.prediction,choices:conf.choices}:conf.tasks[mission].prediction;
 function updatePrediction(){const p=predictionSet();document.querySelector('label[for=prediction]').textContent=p.question;$('prediction').replaceChildren(new Option('請先選一個預測',''),...p.choices.map(([v,l])=>new Option(l,v)));}
 ...
 function renderComparison(){
  ...
  if(records.length<2){region.append(node('p',records.length?'已完成第一筆。只改一個條件，再預測、操作與保存第二筆。':'保存兩筆觀察後，這裡會並排顯示證據。'));return}
  ...
  const pair=node('div','');pair.className='comparison-pair';records.slice(-2).forEach((r,i)=>{const card=node('article','');card.append(node('h3','紀錄 '+(records.length-1+i)));for(const [label,value] of [['結果',r.result],['我的解釋',r.explanation]]){const p=node('p','');p.append(node('strong',label+'：'),document.createTextNode(value));card.append(p)}const details=node('details','');details.append(node('summary','檢查條件與預測'),node('p',r.conditions),node('p','預測：'+r.prediction));card.append(details);pair.append(card)});region.append(pair,node('p','先檢查兩筆是否只差一個條件，再說讀值如何改變；若同時改多個條件，還不能直接歸因。'));
 }
 ...
 function invalidate(message='條件已更新，請重新預測後查看結果。'){
  ...
  current=null;$('prediction').value='';$('add-record').disabled=true;$('diagram').innerHTML='<p class="empty">先選擇預測，再按「操作並觀察」。結果尚未揭露。</p>';$('readouts').replaceChildren();$('current-conditions').textContent='';$('print-current').textContent='本輪尚未操作；請參閱下方已儲存紀錄。';$('result-explanation').textContent='操作後，這裡會說明模型結果。';status(message+($('explanation-input').value?' 你的解釋草稿已保留，重新觀察後再確認內容。':''));labels();
 }
 function run(){if(!$('prediction').value){status('請先選擇一個預測；猜錯也能幫助學習。',true);$('prediction').focus();return}let s;try{s=state()}catch(_){status('控制值無效，請重新設定。',true);return}
  ...
  const p=predictionSet(),answer=taskAnswer(conf.id,s,r,p),correct=p.choices.find(c=>c[0]===answer)?.[1]||answer;current.predictionLabel=p.choices.find(c=>c[0]===current.prediction)?.[1]||current.prediction;current.question=p.question;status(($('prediction').value===answer?'預測符合本模型。':'預測和本模型不同，可以回頭比較。')+' 觀察：'+correct+'。請寫下依據並加入紀錄。');send('start');
  hasObserved=true;const explore=$('moon-explore')||$('lab-explore');if(explore)explore.disabled=false;
 }
 function previewMoon(){const s=state(),r=calculate(conf.id,s),view=describe(conf.id,s,r);current=null;$('prediction').value='';$('add-record').disabled=true;$('diagram').innerHTML=diagram(conf.id,s,r);$('readouts').replaceChildren();view.metrics.forEach(([name,value])=>{const box=node('div',name);box.className='meter';box.append(node('strong',value));$('readouts').append(box)});$('current-conditions').textContent='自由觀察條件：'+conditions(s);$('result-explanation').textContent=view.explanation;labels();status('自由觀察中：拖動滑桿，圖像與讀值會同步變化。要保留紀錄，先關閉自由觀察，再預測與操作。');}
 ...
 if(conf.id!=='moon-eclipse'){
  const label=node('label',''),check=node('input','');check.type='checkbox';check.id='lab-explore';check.disabled=true;label.append(check,document.createTextNode('自由觀察：拖曳即時更新（先完成一次預測）'));label.className='coach';$('diagram').before(label);check.addEventListener('change',()=>{moonExplore=check.checked;if(moonExplore)previewMoon();else invalidate('已離開自由觀察，請重新預測後保存紀錄。')});
 }
 ...
 if(['optics','energy'].includes(conf.id)){
  const tip=node('p','完成一次預測，再開啟自由觀察：可拖曳圖中的橘色物體；鍵盤選中物體後按左右鍵也可以。');tip.className='lab-drag-tip';$('diagram').before(tip);
  ...
  stage.addEventListener('pointerdown',e=>{const handle=e.target.closest('[data-lab-drag]');if(!handle||e.button!==0)return;if(!moonExplore){status('先開啟自由觀察，就能拖動物體；正式紀錄仍要先預測。',true);return}e.preventDefault();const svg=handle.ownerSVGElement,box=svg.getBoundingClientRect();drag={pointer:e.pointerId,key:handle.dataset.labDrag,value:+$('ctl-'+handle.dataset.labDrag).value,x:e.clientX,ratio:+handle.dataset.unitsPerPx*660/Math.max(1,box.width)};try{stage.setPointerCapture(e.pointerId)}catch(_){}});
 ...
 $('reset').addEventListener('click',()=>{conf.controls.forEach(c=>$('ctl-'+c.key).value=c.value);mission=null;updatePrediction();document.querySelectorAll('[data-mission]').forEach(b=>b.removeAttribute('aria-current'));$('mission-prompt').textContent='自由探索：先固定其他條件，只改一個變因。';invalidate('本輪已重新開始，既有紀錄保留。')});
 document.querySelectorAll('[data-mission]').forEach(b=>b.addEventListener('click',()=>{mission=Number(b.dataset.mission);updatePrediction();const t=conf.tasks[mission];conf.controls.forEach(c=>$('ctl-'+c.key).value=Object.hasOwn(t.values,c.key)?t.values[c.key]:c.value);document.querySelectorAll('[data-mission]').forEach(x=>x.removeAttribute('aria-current'));b.setAttribute('aria-current','true');$('mission-prompt').textContent=t.prompt;invalidate('已載入任務條件。預測由你選，不會自動填答案。');$('prediction').focus()}));
 $('add-record').addEventListener('click',()=>{if(!current)return;const explanation=$('explanation-input').value.trim();if(explanation.length<2){status('請先寫下你觀察到的數據或關係，再加入紀錄。',true);$('explanation-input').focus();return}
  records.push({settings:{...current.s},question:current.question,conditions:conditions(current.s),prediction:current.predictionLabel,result:current.view.metrics.map(m=>m.join('：')).join('；'),explanation:explanation.slice(0,1000)});records=records.slice(-50);save();renderRecords();$('add-record').disabled=true;send('record');if(current.mission!==null)send('task_complete');status('已加入第'+records.length+'筆紀錄。請只改一個條件，再做一次比較。')});
 $('clear-records').addEventListener('click',()=>{if(!confirm('清除本頁在此瀏覽器的全部觀察紀錄？'))return;records=[];save();renderRecords();status('紀錄已清除。')});
 function preparePrint(){$('print-current').textContent=current?'本輪預測：'+current.predictionLabel+'；本輪解釋（含尚未存入紀錄的文字）：'+($('explanation-input').value.trim()||'尚未填寫。'):'本輪尚未操作；請參閱下方已儲存紀錄。'}
