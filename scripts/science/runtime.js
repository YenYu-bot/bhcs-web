(() => {
 ...
 function renderRecords(){const body=$('records'),printList=$('print-records');body.replaceChildren();printList.replaceChildren();if(!records.length){const tr=node('tr',''),td=node('td',noPrediction?'尚未記錄；先操作並留下第一筆觀察。':'尚未記錄；先完成預測與操作。');td.colSpan=noPrediction?4:5;tr.append(td);body.append(tr);printList.append(node('p','尚無已儲存紀錄。'))}else records.forEach((r,i)=>{const tr=node('tr','');(noPrediction?[String(i+1),r.conditions,r.result,r.explanation]:[String(i+1),r.conditions,r.prediction,r.result,r.explanation]).forEach(v=>tr.append(node('td',v)));body.append(tr);const card=node('article','');card.className='print-record';card.append(node('h3','紀錄 '+(i+1)));(noPrediction?[['條件',r.conditions],['結果',r.result],['解釋',r.explanation]]:[['條件',r.conditions],['預測',r.prediction],['結果',r.result],['解釋',r.explanation]]).forEach(([label,value])=>{const p=node('p','');p.append(node('strong',label+'：'),document.createTextNode(value));card.append(p)});printList.append(card)});$('storage-status').textContent=saveNotice||'僅保存於本機，最多50筆，不收集姓名。';renderComparison()}
 function renderComparison(){
  const region=$('record-comparison');region.replaceChildren();region.append(node('h3','最近兩筆，放在一起比較'));
  if(records.length<2){region.append(node('p',records.length?(noPrediction?'已完成第一筆。只改一個條件，再操作並保存第二筆。':'已完成第一筆。只改一個條件，再預測、操作與保存第二筆。'):'保存兩筆觀察後，這裡會並排顯示證據。'));return}
  const [a,b]=records.slice(-2);if(a.settings&&b.settings){const keys=conf.controls.filter(c=>Object.hasOwn(a.settings,c.key)&&Object.hasOwn(b.settings,c.key)&&a.settings[c.key]!==b.settings[c.key]);const dif=node('p',keys.length===1?'這次只改了：':'比較提醒：'+(keys.length===0?'兩筆設定相同。':'同時改了 '+keys.length+' 個設定，不能直接歸因。'));dif.className='lab-diff';for(const ctl of keys){const format=v=>ctl.options?ctl.options.find(o=>o[0]===v)?.[1]||String(v):v+' '+ctl.unit;dif.append(node('strong',ctl.label+' '+format(a.settings[ctl.key])+' → '+format(b.settings[ctl.key])+'；'))}region.append(dif)}else region.append(node('p','舊紀錄沒有結構化條件，請展開原始條件自行比較。'));
  const pair=node('div','');pair.className='comparison-pair';records.slice(-2).forEach((r,i)=>{const card=node('article','');card.append(node('h3','紀錄 '+(records.length-1+i)));for(const [label,value] of [['結果',r.result],['我的解釋',r.explanation]]){const p=node('p','');p.append(node('strong',label+'：'),document.createTextNode(value));card.append(p)}const details=node('details','');details.append(node('summary',noPrediction?'檢查實驗條件':'檢查條件與預測'),node('p',r.conditions));if(!noPrediction)details.append(node('p','預測：'+r.prediction));card.append(details);pair.append(card)});region.append(pair,node('p','先檢查兩筆是否只差一個條件，再說讀值如何改變；若同時改多個條件，還不能直接歸因。'));
 }
 ...
 function run(){if(!noPrediction&&!$('prediction').value){status('請先選擇一個預測；猜錯也能幫助學習。',true);$('prediction').focus();return}let s;try{s=state()}catch(_){status('控制值無效，請重新設定。',true);return}
  ...
  requestAnimationFrame(()=>$('diagram').scrollIntoView?.({block:'start',behavior:'auto'}));
 }
 ...
 if(conf.id==='moon-eclipse'){
  ...
  const label=node('label',''),check=node('input','');check.type='checkbox';check.id='moon-explore';check.disabled=true;label.append(check,document.createTextNode(noPrediction?'自由觀察：拖曳滑桿即時預覽（先完成一次正式觀察）':'自由觀察：拖曳滑桿即時預覽（先完成一次預測）'));label.className='coach';$('diagram').before(label);check.addEventListener('change',()=>{moonExplore=check.checked;if(moonExplore)previewMoon();else invalidate(noPrediction?'已離開自由觀察。按「開始觀察」即可保留新的紀錄。':'已離開自由觀察。請選預測，再操作並保留紀錄。')});
 }
 if(conf.id!=='moon-eclipse'){
  const label=node('label',''),check=node('input','');check.type='checkbox';check.id='lab-explore';check.disabled=true;label.append(check,document.createTextNode('自由觀察：拖曳或調整控制即時更新（先完成一次正式觀察）'));label.className='coach';$('diagram').before(label);check.addEventListener('change',()=>{moonExplore=check.checked;$('diagram').classList.toggle('is-exploring',moonExplore);if(moonExplore)previewMoon();else invalidate('已離開自由觀察。按「開始觀察」即可保留新的紀錄。')});
 }
 if(['optics','energy'].includes(conf.id)){
  const tip=node('p','完成一次正式觀察後，可開啟自由觀察並拖曳圖中的橘色物體；鍵盤選中物體後按左右鍵也可以。');tip.className='lab-drag-tip';$('diagram').before(tip);
  ...
  stage.addEventListener('pointerdown',e=>{const handle=e.target.closest('[data-lab-drag]');if(!handle||e.button!==0)return;if(!moonExplore){status('先完成一次正式觀察並開啟自由觀察，就能拖動物體。',true);return}e.preventDefault();const svg=handle.ownerSVGElement,box=svg.getBoundingClientRect();drag={pointer:e.pointerId,key:handle.dataset.labDrag,value:+$('ctl-'+handle.dataset.labDrag).value,x:e.clientX,ratio:+handle.dataset.unitsPerPx*660/Math.max(1,box.width)};try{stage.setPointerCapture(e.pointerId)}catch(_){}});
  ...
  stage.addEventListener('keydown',e=>{const h=e.target.closest('[data-lab-drag]');if(!h||!['ArrowLeft','ArrowRight'].includes(e.key))return;e.preventDefault();if(!moonExplore){status('先完成一次正式觀察並開啟自由觀察，再移動物體。',true);return}const key=h.dataset.labDrag,control=conf.controls.find(c=>c.key===key);setValue(key,+$('ctl-'+key).value+(e.key==='ArrowRight'?1:-1)*Math.sign(+h.dataset.unitsPerPx)*control.step);stage.querySelector('[data-lab-drag]')?.focus();});
 }
 if($('prediction'))$('prediction').addEventListener('change',()=>{if(current){const selected=$('prediction').value;invalidate('預測已變更，請重新操作。');$('prediction').value=selected}});
 ...
 document.querySelectorAll('[data-mission]').forEach(b=>b.addEventListener('click',()=>{mission=Number(b.dataset.mission);const t=conf.tasks[mission];conf.controls.forEach(c=>$('ctl-'+c.key).value=Object.hasOwn(t.values,c.key)?t.values[c.key]:c.value);document.querySelectorAll('[data-mission]').forEach(x=>x.removeAttribute('aria-current'));b.setAttribute('aria-current','true');$('mission-prompt').textContent=t.prompt;invalidate(noPrediction?'已載入任務條件。直接操作並觀察變化。':'已載入任務條件。預測由你選，不會自動填答案。');if($('prediction'))$('prediction').focus();else $('run').focus()}));
 $('add-record').addEventListener('click',()=>{if(!current)return;const explanation=$('explanation-input').value.trim();if(explanation.length<2){status('請先寫下你觀察到的數據或關係，再加入紀錄。',true);$('explanation-input').focus();return}
  records.push({settings:{...current.s},conditions:conditions(current.s),prediction:noPrediction?'':conf.choices.find(c=>c[0]===current.prediction)[1],result:current.view.metrics.map(m=>m.join('：')).join('；'),explanation:explanation.slice(0,1000)});records=records.slice(-50);save();renderRecords();$('add-record').disabled=true;send('record');if(current.mission!==null)send('task_complete');status('已加入第'+records.length+'筆紀錄。請只改一個條件，再做一次比較。')});
 $('clear-records').addEventListener('click',()=>{if(!confirm('清除本頁在此瀏覽器的全部觀察紀錄？'))return;records=[];save();renderRecords();status('紀錄已清除。')});
 function preparePrint(){$('print-current').textContent=current?(noPrediction?'本輪解釋（含尚未存入紀錄的文字）：'+($('explanation-input').value.trim()||'尚未填寫。'):'本輪預測：'+conf.choices.find(c=>c[0]===current.prediction)[1]+'；本輪解釋（含尚未存入紀錄的文字）：'+($('explanation-input').value.trim()||'尚未填寫。')):'本輪尚未操作；請參閱下方已儲存紀錄。'}
