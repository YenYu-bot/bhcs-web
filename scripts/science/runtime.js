(() => {
 ...
 function renderRecords(){const body=$('records');body.replaceChildren();if(!records.length){const tr=node('tr',''),td=node('td','尚未記錄；先完成預測與操作。');td.colSpan=5;tr.append(td);body.append(tr)}else records.forEach((r,i)=>{const tr=node('tr','');[String(i+1),r.conditions,r.prediction,r.result,r.explanation].forEach(v=>tr.append(node('td',v)));body.append(tr)});$('storage-status').textContent=saveNotice||'僅保存於本機，最多50筆，不收集姓名。'}
 ...
 function labels(){
  const mode=$('ctl-mode')?.value;
  const active=conf.id==='electromagnetism'?({magnet:['mode','turns','current'],motor:['mode','turns','current','field','angle'],generator:['mode','turns','field','angle','rpm']})[mode]:conf.id==='pressure-fluid'?({water:['mode','depth','density'],gas:['mode','volume'],flow:['mode','density','ratio','speed']})[mode]:null;
 ...
 function invalidate(message='條件已更新，請重新預測後查看結果。'){
  current=null;$('prediction').value='';$('add-record').disabled=true;$('explanation-input').value='';$('diagram').innerHTML='<p class="empty">先選擇預測，再按「操作並觀察」。結果尚未揭露。</p>';$('readouts').replaceChildren();$('result-explanation').textContent='操作後，這裡會說明模型結果。';status(message);labels();
 }
 function run(){if(!$('prediction').value){status('請先選擇一個預測；猜錯也能幫助學習。',true);$('prediction').focus();return}let s;try{s=state()}catch(_){status('控制值無效，請重新設定。',true);return}
  ...
  $('result-explanation').textContent=view.explanation;$('add-record').disabled=false;
 ...
 $('add-record').addEventListener('click',()=>{if(!current)return;const explanation=$('explanation-input').value.trim();if(explanation.length<2){status('請先寫下你觀察到的數據或關係，再加入紀錄。',true);$('explanation-input').focus();return}
  const conditions=conf.controls.map(c=>`${c.label}：${c.options?c.options.find(o=>o[0]===current.s[c.key])[1]:current.s[c.key]+' '+c.unit}`).join('；');
  records.push({conditions,prediction:conf.choices.find(c=>c[0]===current.prediction)[1],result:current.view.metrics.map(m=>m.join('：')).join('；'),explanation:explanation.slice(0,1000)});records=records.slice(-50);save();renderRecords();$('add-record').disabled=true;send('record');if(current.mission!==null)send('task_complete');status('已加入第'+records.length+'筆紀錄。請只改一個條件，再做一次比較。')});
 ...
 document.querySelectorAll('.quiz input').forEach(el=>el.addEventListener('change',()=>{quizSent=false;$('quiz-score').textContent='答案已變更，請重新檢查。'}));
