(()=>{
 ...
 const welcome=make('div','','lab-welcome'),picture=make('div','','lab-welcome-picture');picture.style.backgroundImage=`url("${conf.img}lab-room.jpg")`;picture.append(img('doc-wave.png','奇奇博士歡迎你'));const copy=make('div','','lab-welcome-copy');copy.append(make('span','今天的研究問題','lab-kicker'),make('h2',question),make('p','嗨，我是奇奇博士。直接動手找證據，把看見的變化收進研究手冊。'),make('p',steps[0]),make('p','本次挑戰：'+conf.inquiry[3],'lab-task-rule'),button('接受任務，進入操作台 →',()=>show('bench'),'lab-primary'),make('p','免登入 · 不用填姓名 · 可隨時回到前一步','lab-promise'));welcome.append(picture,copy);screens.prepare.append(welcome);
 const coach=make('aside','','lab-coach'),doc=img('doc-point.png','奇奇博士'),speech=make('div'),coachTitle=make('strong','奇奇博士 · 現在這樣做'),coachText=make('p',steps[0]),hint=make('p','先從目前條件開始操作；看完結果，再決定下一次要改哪一個條件。','lab-hint');speech.append(coachTitle,coachText,hint);coach.append(doc,speech);screens.bench.append(coach);
 ...
 function refresh(){if(!document||!main.isConnected)return;const n=countRecords(),score=$('quiz-score')||$('quizScore'),passed=!!score&&/答對\s*3\s*[／/]\s*3/.test(score.textContent);badgeStatus.textContent=n>=2&&passed?'研究活動完成！':'研究徽章等待解鎖';badgeDetails.textContent=`觀察紀錄 ${Math.min(2,n)}／2 · 三題檢核${passed?'已通過':'尚未通過'}。${n>=2&&passed?'再用自己的話說明兩筆證據的差異。':'完成兩筆觀察並檢查三題答案，再回來看看。'}`;const source=$('addRecord'),quick=$('lab-save-observation');if(quick)quick.disabled=source.disabled;
  if(n>=2){coachText.textContent=steps[2];doc.src=conf.img+'doc-thumb.png'}else if(n===1){coachText.textContent='第一筆已收好。'+conf.inquiry[1]+' 再做一次操作與觀察。';doc.src=conf.img+'doc-point.png'}
 }
