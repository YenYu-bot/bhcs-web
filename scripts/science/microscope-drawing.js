...
function drawMicroCells(kind,ctx=microLayerContext){
 ...
 if(kind==='letter'){
  const limits=MICRO_SPECIMEN_BOUNDS.letter;ctx.save();ctx.beginPath();ctx.rect(-limits.halfX,-limits.halfY,limits.halfX*2,limits.halfY*2);ctx.clip();ctx.scale(u,u);ctx.fillStyle='rgba(36,61,66,.92)';ctx.font='700 1200px Georgia,serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('e',420,-160);ctx.strokeStyle='rgba(190,72,55,.78)';ctx.lineWidth=4/(u*z);ctx.beginPath();ctx.moveTo(-350,0);ctx.lineTo(350,0);ctx.moveTo(0,-350);ctx.lineTo(0,350);ctx.stroke();ctx.restore();return;
 }
 const {w,h}=MICRO_SIZES[kind],limits=MICRO_SPECIMEN_BOUNDS[kind];
 const col0=Math.floor(b.minX/u/w)-2,col1=Math.ceil(b.maxX/u/w)+2,row0=Math.floor(b.minY/u/h)-2,row1=Math.ceil(b.maxY/u/h)+2;
 ctx.save();ctx.beginPath();ctx.rect(-limits.halfX,-limits.halfY,limits.halfX*2,limits.halfY*2);ctx.clip();ctx.scale(u,u);ctx.lineJoin='round';
...
function drawScope(){
 const canvas=$('scope'),obj=objective(),z=obj/4,light=+els.light.value/100,magDim=obj===4?1:obj===10?.94:.84,bright=Math.max(.24,light*magDim),c=clarity(),inView=microSpecimenInView(els.specimen.value,obj,+els.stageX.value,+els.stageY.value);
 ...
 const layer=microLayerContext;layer.clearRect(0,0,600,600);layer.fillStyle=baseBackground(bright);layer.fillRect(0,0,600,600);layer.save();layer.translate(300,300);layer.scale(-z,-z);layer.translate(+els.stageX.value*MICRO_STAGE_GAIN,-(+els.stageY.value*MICRO_STAGE_GAIN));drawMicroCells(els.specimen.value,layer);layer.restore();ctx.fillStyle=baseBackground(bright);ctx.fillRect(0,0,600,600);ctx.filter=`blur(${((100-c)/18).toFixed(2)}px)`;ctx.drawImage(microLayer,0,0);ctx.restore();ctx.filter='none';
 ...
 if(!inView){ctx.fillStyle='rgba(25,52,62,.82)';ctx.fillRect(130,246,340,108);ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font='800 24px system-ui, Microsoft JhengHei';ctx.fillText('試片已移出視野',300,288);ctx.font='600 17px system-ui, Microsoft JhengHei';ctx.fillText('請將試片移回中央再觀察',300,324);ctx.textAlign='start';}
 if(c<28&&inView){ctx.fillStyle='rgba(16,42,67,.7)';ctx.fillRect(185,270,230,58);ctx.fillStyle='#fff';ctx.font='800 22px Microsoft JhengHei';ctx.textAlign='center';ctx.fillText('影像離焦，請調整焦距',300,307);ctx.textAlign='start';}ctx.restore();
 canvas.setAttribute('aria-label',`程式繪製示意，非顯微照片。${specimens[els.specimen.value].name}，${inView?'試片在視野內':'試片已移出視野'}，總倍率 ${10*obj} 倍，視野直徑 ${fmt(fov(obj))} 毫米，比例尺 ${bar.label}，清晰度 ${c}%`);updateReadouts();updateSlideBoard();
}
