...
function microPath(points){ctx.beginPath();points.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.closePath();}
function drawMicroCells(kind){
 ...
 for(let row=row0;row<=row1;row++)for(let col=col0;col<=col1;col++){
  ...
  microPath(cell.points);ctx.fillStyle=kind==='onion'?`rgba(217,171,70,${.09+cell.tone*.07})`:kind==='cheek'?`rgba(109,153,207,${.08+cell.tone*.10})`:`rgba(126,169,72,${.07+cell.tone*.08})`;ctx.fill();
  ...
  if(kind==='elodea'&&z>=2.5){ctx.save();microPath(cell.points);ctx.clip();for(const p of cell.organelles){ctx.beginPath();ctx.ellipse(p.x,p.y,p.rx,p.ry,p.angle,0,Math.PI*2);ctx.fillStyle=`rgba(${55+Math.round(p.tone*22)},${110+Math.round(p.tone*32)},47,${.55+p.tone*.30})`;ctx.fill();}ctx.restore();}
 }
...
function drawScope(){
 ...
 ctx.fillStyle=baseBackground(bright);ctx.fillRect(0,0,600,600);ctx.filter=`blur(${((100-c)/18).toFixed(2)}px)`;ctx.translate(300,300);ctx.scale(-z,-z);ctx.translate(+els.stageX.value,-(+els.stageY.value));drawMicroCells(els.specimen.value);ctx.restore();ctx.filter='none';
 ...
 canvas.setAttribute('aria-label',`程式繪製示意，非顯微照片。${specimens[els.specimen.value].name}，總倍率 ${10*obj} 倍，視野直徑 ${fmt(fov(obj))} 毫米，比例尺 ${bar.label}，清晰度 ${c}%`);updateReadouts();
}
