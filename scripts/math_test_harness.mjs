...
export function auditCombination({unit,ctx,uiMax,safeQuestion,contentGuard=()=>true,resetRandom,seed}) {
 ...
 if(result.bankDeclared)try{const n=typeof unit.bankSize==='function'?unit.bankSize(ctx):unit.bankSize;if(!Number.isSafeInteger(n)||n<1)issue('invalid_bank_size','bankSize must resolve to a positive safe integer');else result.bankSize=n;}catch(e){issue('bank_size_exception',e.message);}
 ...
 for(let i=0;i<result.paper.requested;i++){
  ...
  if(q==null){result.paper.exhaustedAt=i+1;issue(result.bankSize===null?'undeclared_bank_or_generation_exhausted':'declared_bank_generation_exhausted',`safeQuestion returned null at ${i+1}/${result.paper.requested} after 50 attempts`);break;}
