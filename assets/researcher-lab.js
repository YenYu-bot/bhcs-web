...
function show(id,options={}){const target=q('.researcher-screen[data-screen="'+id+'"]');if(!target)return;const previous=activeScreen(),push=options.push!==false;qa('.researcher-screen').forEach(s=>{const on=s===target;s.classList.toggle('is-active',on);s.setAttribute('aria-hidden',String(!on))});qa('.researcher-nav button').forEach(b=>b.setAttribute('aria-current',b.dataset.to===id?'step':'false'));if(push&&previous&&previous!==id)history.pushState(stateFor(id),'');else if(options.replace)history.replaceState(stateFor(id),'');scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});if(options.focus!==false)requestAnimationFrame(()=>{if(!target.contains(document.activeElement))screenHeading(id)?.focus({preventScroll:true})});try{sessionStorage.setItem('bhcs-researcher-screen:'+path,id)}catch(_){ }updateNotebookReturn()}
